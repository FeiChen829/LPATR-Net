...
def _train(model, args):
    ...
    dataloader = train_dataloader(args.data_dir, args.batch_size, args.num_worker,DatasetName=args.DatasetName)
    ...
    for epoch_idx in range(epoch, args.num_epoch + 1):
        ...
        for iter_idx, batch_data in enumerate(dataloader):
            ...
            label_img2 = F.interpolate(label_img, scale_factor=0.5, mode='bicubic')
            label_img4 = F.interpolate(label_img, scale_factor=0.25, mode='bicubic')
            ...
            # tensorboard
            writer.add_scalar('Pixel Loss', loss_content.item(), iter_idx + (epoch_idx - 1) * max_iter)
            writer.add_scalar('FFT Loss', loss_fft.item(), iter_idx + (epoch_idx - 1) * max_iter)
            writer.add_scalar('Total Loss', loss.item(), iter_idx + (epoch_idx - 1) * max_iter)
        ...
        save_name = os.path.join(args.model_save_dir, 'model_now.pkl')
        checkpoint = {
        ...
        # tensorboard
        writer.add_scalar('PSNR_val', psnr, epoch_idx)
        writer.add_scalar('SSIM_val', ssim, epoch_idx)
        ...
        print('PSNR  %.2f dB' % (best_psnr))
        print('SSIM  %.4f dB' % (best_ssim))
        print('↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑')
